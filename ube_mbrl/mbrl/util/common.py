...
def create_one_dim_tr_model(
    ...
    if model_cfg._target_ == "ube_mbrl.mbrl.models.BasicEnsemble":
    ...
    if name_obs_process_fn:
        ...
        obs_process_fn = None
    dynamics_model = ube_mbrl.mbrl.models.OneDTransitionRewardModel(
        model,
...
def create_replay_buffer(
    ...
    """
    dataset_size = cfg.algorithm.get("dataset_size", None) if "algorithm" in cfg else None
    if not dataset_size:
    ...
    if collect_trajectories:
        if cfg.overrides.trial_length is None:
            raise ValueError(
                "cfg.overrides.trial_length must be set when " "collect_trajectories==True."
            )
...
def train_model_and_save_model_and_data(
    model: ube_mbrl.mbrl.models.Model,
    model_trainer: ube_mbrl.mbrl.models.ModelTrainer,
    cfg: omegaconf.DictConfig,
    replay_buffer: ReplayBuffer,
    ...
    """
    dataset_train, dataset_val = ube_mbrl.mbrl.util.common.get_basic_buffer_iterators(
        replay_buffer,
...
def rollout_model_env(
    model_env: ube_mbrl.mbrl.models.ModelEnv,
    initial_obs: np.ndarray,
    plan: Optional[np.ndarray] = None,
    agent: Optional[ube_mbrl.mbrl.planning.Agent] = None,
    num_samples: int = 1,
