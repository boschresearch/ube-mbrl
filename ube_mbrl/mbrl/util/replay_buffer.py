...
class ReplayBuffer:
    ...
    REPLAY_FNAME = "replay_buffer.pth"
    ...
    def sample_subsequence(self, batch_size: int, seq_len: int) -> List[TransitionBatch]:
        """ """
        indices = self._rng.choice(self.num_stored, size=batch_size)
        batches = []
        for step in range(seq_len):
            batch = self._batch_from_indices(indices)
            is_final = indices == self.num_stored - 1
            batch.dones |= is_final
            batches.append(batch)
            if batch.dones.all():
                break
            indices = indices[~batch.dones]
            indices += 1
        return batches
    ...
    def deep_save(
        ...
        fname: Optional[str] = REPLAY_FNAME,
    ):
        """Does a deep copy of the entire instance into a file.

        Args:
            save_dir

            rollout_batch_size: in the case of model rollouts, this input represents that the
            initial rollout_batch_size observations come from the true environment state
            distribution. We use it for plotting purposes.

            rollout_length: in the case of model rollouts, it denotes the length of the model-based
            rollouts. Used as metadata for plotting and logging.

            fname: optional name to give to the file
        """
        self.rollout_batch_size = rollout_batch_size
        self.rollout_length = rollout_length
        path = pathlib.Path(save_dir) / fname
