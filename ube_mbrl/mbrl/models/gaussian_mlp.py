...
class GaussianMLP(Ensemble):
    ...
    def __init__(
        ...
        hidden_layers = [nn.Sequential(create_linear_layer(in_size, hid_size), create_activation())]
    ...
    def forward(  # type: ignore
        ...
        """
        if use_propagation:
            return self._forward_ensemble(x, rng=rng, propagation_indices=propagation_indices)
    ...
    def _nll_loss(self, model_in: torch.Tensor, target: torch.Tensor) -> torch.Tensor:
        ...
        nll = (
            ube_mbrl.mbrl.util.math.gaussian_nll(pred_mean, pred_logvar, target, reduce=False)
    ...
    def sample_propagation_indices(self, batch_size: int, _rng: torch.Generator) -> torch.Tensor:
        model_len = len(self.elite_models) if self.elite_models is not None else len(self)
