...
def truncated_normal_init(m: nn.Module):
    ...
    if isinstance(m, nn.Linear):
        ...
        ube_mbrl.mbrl.util.math.truncated_normal_(m.weight.data, std=stddev)
        m.bias.data.fill_(0.0)
    if isinstance(m, EnsembleLinearLayer):
        num_members, input_dim, _ = m.weight.data.shape
        stddev = 1 / (2 * np.sqrt(input_dim)) * 2.0
        for i in range(num_members):
            ube_mbrl.mbrl.util.math.truncated_normal_(m.weight.data[i], std=stddev)
        m.bias.data.fill_(0.0)
...
def to_tensor(x: ube_mbrl.mbrl.types.TensorType):
...
class Conv2dEncoder(nn.Module):
    def __init__(
        ...
        for layer_cfg in layers_config:
            conv_modules.append(
                nn.Sequential(
                    nn.Conv2d(layer_cfg[0], layer_cfg[1], layer_cfg[2], stride=layer_cfg[3]),
                    activation_cls(),
                )
            )
        self.convs = nn.ModuleList(conv_modules)
        cnn_out_size = np.prod(get_cnn_output_size(self.convs, layers_config[0][0], image_shape))
