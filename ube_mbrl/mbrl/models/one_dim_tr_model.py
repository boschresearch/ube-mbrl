...
import ube_mbrl.mbrl.models.util as model_util
...
class OneDTransitionRewardModel(Model):
    ...
    def __init__(
        ...
        obs_process_fn: Optional[ube_mbrl.mbrl.types.ObsProcessFnType] = None,
        no_delta_list: Optional[List[int]] = None,
        num_elites: Optional[int] = None,
    ):
        super().__init__(model.device)
        self.model = model
        self.input_normalizer: Optional[ube_mbrl.mbrl.util.math.Normalizer] = None
        if normalize:
            self.input_normalizer = ube_mbrl.mbrl.util.math.Normalizer(
                self.model.in_size,
                self.model.device,
    ...
    def _get_model_input(
        self,
        obs: ube_mbrl.mbrl.types.TensorType,
        action: ube_mbrl.mbrl.types.TensorType,
    ) -> torch.Tensor:
        if self.obs_process_fn:
            obs = self.obs_process_fn(obs)
        obs = model_util.to_tensor(obs).to(self.device)
        action = model_util.to_tensor(action).to(self.device)
        model_in = torch.cat([obs, action], dim=obs.ndim - 1).float()
    ...
    def _process_batch(
        self, batch: ube_mbrl.mbrl.types.TransitionBatch, _as_float: bool = False
    ...
    def update_normalizer(self, batch: ube_mbrl.mbrl.types.TransitionBatch):
    ...
    def loss(
        self,
        batch: ube_mbrl.mbrl.types.TransitionBatch,
    ...
    def get_output_and_targets(
        self, batch: ube_mbrl.mbrl.types.TransitionBatch
    ...
    def deep_save(self, save_dir: Union[str, pathlib.Path]):
        """Saves the entire model to the given directory."""
        torch.save(self, pathlib.Path(save_dir) / self.MODEL_FNAME)
